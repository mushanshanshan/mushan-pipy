...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        if self.pre_post_layer == 'linear':
            ...
            self.project_in = Sequential(
                nn.Conv1d(in_channels=dim, out_channels=codebook_input_dim, kernel_size=pre_post_kernal, padding=(pre_post_kernal - 1) // 2),
                nn.LayerNorm([codebook_input_dim, 1]) if layernorm_after_project_in else None
            ) if requires_projection else nn.Identity()
            self.project_out = nn.Conv1d(in_channels=codebook_input_dim, out_channels=dim, kernel_size=pre_post_kernal, padding=(pre_post_kernal - 1) // 2) if requires_projection else nn.Identity()
    ...
    def forward(
        ...
        if self.pre_post_layer == 'conv':
            quantize = self.project_out(quantize.transpose(1,2)).transpose(1,2)
            x = self.project_in(x.transpose(1,2)).transpose(1,2)
        else:
    ...
    def __init__(self, config):
        ...
        for para in config.train.model_list:
            self.vqs['_'.join([str(i) for i in para])] = VectorQuantize(
                                                        ...
                                                        use_cosine_sim = para[0] == 'cos',
                                                        # pre_post_layer_type = para[3],
                                                        # pre_post_kernal = para[4],
                                                        decay = 0.8,             
