...
class EMA(nn.Module):
    ...
    def __init__(
        ...
        if not exists(self.ema_model):
            try:
                ...
                print('Your model was not copyable. Using save trick')
                torch.save(model, "./ema_temp.pth")
                self.ema_model = torch.load("./ema_temp.pth")
                os.remove("./ema_temp.pth")
    ...
    def update_moving_average(self, ma_model, current_model):
        ...
        for (name, current_buffer), (_, ma_buffer) in zip(self.get_buffers_iter(current_model), self.get_buffers_iter(ma_model)):
            ...
            if self.force_cpu:
                current_buffer = current_buffer.cpu()
